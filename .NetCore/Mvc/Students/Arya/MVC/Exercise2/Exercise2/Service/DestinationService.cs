...
namespace Exercise2.Service
{
    public class DestinationService:IDestinationService
    {
        ...
        public async Task<DestinationResponseDto?> PatchAsync(Guid id, DestinationPathDto pathDto)
        {
            var dest = await _repo.GetByIdAsync(id);
            if (dest == null) return null;

            if (pathDto.Name != null) dest.Name = pathDto.Name;
            if (pathDto.City != null) dest.City = pathDto.City;

            if (pathDto.ImageFile != null)
            {
                if (!string.IsNullOrEmpty(dest.ImageUrl))
                    TryDeleteFile(dest.ImageUrl);
                dest.ImageUrl = await SaveImage(pathDto.ImageFile);
            }
