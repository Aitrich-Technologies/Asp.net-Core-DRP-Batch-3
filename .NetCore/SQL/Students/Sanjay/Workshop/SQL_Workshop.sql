...
INSERT INTO Destinations ([Name], City, [Description], CreatedBy) 
VALUES ( 'Grand Canyon Helicopter Tour', 'Las Vegas',
'A luxury flight over the West Rim with a champagne toast.',
(SELECT Id FROM [Users]
WHERE UserName = 'skyhigh_travel') );

INSERT INTO Destinations ([Name], City, [Description], CreatedBy) 
VALUES ( 'Drifting', 'Tokyo',
'A luxury flight over the West Rim with a champagne toast.',
(SELECT Id FROM [Users]
WHERE UserName = 'skyhigh_travel') );
...
DELETE FROM Destinations WHERE [Name] = 'Drifting';
 -- Verification: Check that the destination is gone 
SELECT * FROM Destinations;


ALTER TABLE Destinations ADD Description varchar(30);
